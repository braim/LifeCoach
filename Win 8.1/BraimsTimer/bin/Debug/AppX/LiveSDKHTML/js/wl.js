...
wl_app.createDownload = function (properties) {
    properties.create = true;
    return wl_app.download(properties);
};
...
wl_app.createUpload = function (properties) {
    properties.create = true;
    return wl_app.upload(properties);
};
...
WL.backgroundDownload = function (properties, callback) {
    /// <summary>
    /// Makes a call to download a file from OneDrive. This is an async method that returns a Promise object that 
...
WL.createBackgroundDownload = function (properties, callback) {
    /// <summary>
    /// Creates a background transfer operation to download a file from OneDrive. This is an async method that returns a Promise object that 
    /// allows you to attach events to handle succeeded and failed.
    /// </summary>
    /// <param name="properties" type="Object">Required. A JSON object containing the properties for downloading a file:
    /// &#10; path: Required. The path to the file to download.
    /// &#10; file_output: Optional. The file output object where to write the downloaded file data.
    ...
    try {
        var method = "WL.createBackgroundDownload",
            args = normalizeArguments(arguments, method);
        return wl_app.createDownload(args);
    }
    catch (e) {
        return handleAsyncCallingError(method, e);
    }
...
WL.backgroundUpload = function (properties, callback) {
    /// <summary>
    /// Makes a call to upload a file to OneDrive. This is an async method that returns a Promise object that 
    /// allows you to attach events to handle succeeded, failed and progressed situations.
    /// </summary>
...
WL.createBackgroundUpload = function (properties, callback) {
    /// <summary>
    /// Create an background transfer operation to upload a file to OneDrive. This is an async method that returns a Promise object that 
    /// allows you to attach events to handle succeeded and failed situations.
    /// </summary>
    /// <param name="properties" type="Object">Required. A JSON object containing the properties for uploading a file:
    /// &#10; path: Required. The path to the file to download.
    ...
    try {
        var method = "WL.createBackgroundUpload",
            args = normalizeArguments(arguments, method);
        return wl_app.createUpload(args);
    }
    catch (e) {
        return handleAsyncCallingError(method, e);
    }
...
WL.getCurrentBackgroundDownloads = function () {
    /// <summary>
    /// Gets current pending background download operations. It returns a promise object that allows you to handle 
    /// a succeeded event where you will get a list of current pending download operations. You can invoke attach()
    /// method on each operation. The attach() method returns a promise object that allow you to handle corresponding 
    /// onSuccess, onFailure, and onProgress event for that operation. 
    /// </summary>
    return wl_app.getCurrentBackgroundDownloads();
};

WL.getCurrentBackgroundUploads = function () {
    /// <summary>
    /// Gets current pending background upload operations. It returns a promise object that allows you to handle 
    /// a succeeded event where you will get a list of current pending upload operations. You can invoke attach()
    /// method on each operation. The attach() method returns a promise object that allow you to handle corresponding 
    /// onSuccess, onFailure, and onProgress event for that operation. 
    /// </summary>
    return wl_app.getCurrentBackgroundUploads();
}
...
function executeDownload(op) {
    var props = op._properties,
        ...
        btOp = props.pendingOp;
        startDownloadViaBackgroundTransfer(path, file_output, op, props.create, btOp);
}

function startDownloadViaBackgroundTransfer(path, file_output, op, createOp, btOp) {
    ...
    try {
        ...
        if (btOp) {
            ...
            if (op.started) {
                asyncOp = btOp.attachAsync();
            }
            else {
                asyncOp = btOp.startAsync();
            }
        }
        else {
            ...
            if (createOp) {
                op.downloadComplete(
                    true,
                    new BackgroundOperationWrapper(
                            new DownloadOperation({
                                pendingOp: downloadOp,
                                started: false,
                                interface_method: "BackgroundDownload.start"
                            },
                            downloadOp.guid)));
                return; // This is the end of createBackgroundDownload
            }
            asyncOp = downloadOp.startAsync();
        }

        var promise = asyncOp.then(
            function (response) {
                var appResp = {
...
wl_app.getCurrentBackgroundDownloads = function () {
    var promise = new Promise("getCurrentBackgroundDownloads", null, null);
            ...
            if (response && response.size > 0) {
                for (i = 0; i < response.size; i++) {
                    downloads.push(
                        new BackgroundOperationWrapper(
                            new DownloadOperation({
                                pendingOp: response[i],
                                started: true,
                                interface_method: "BackgroundDownload.attach"
                            }),
                            response[i].guid)
                    );
                }
            }
            promise[PROMISE_EVENT_ONSUCCESS](downloads);
        },
        function (error) {
            promise[PROMISE_EVENT_ONERROR](error);
        }
    );
...
function BackgroundOperationWrapper(operation, guid) {
    this.guid = guid;
...
UploadOperation.prototype._getStrategy = function (props) {
    var interfaceMethod = props[API_INTERFACE_METHOD],
        ...
        btOp = props.pendingOp;
    if (btOp) {
        return new BackgroundTransferUploadStrategy(this, file, props.create, btOp);
    }
    else if (file) {
        if (file instanceof Windows.Storage.StorageFile) {
            return new BackgroundTransferUploadStrategy(this, file, props.create, null);
        } else {
        ...
        return new BackgroundTransferUploadStrategy(this, stream, props.create, null);
    }
 ...
 */
function BackgroundTransferUploadStrategy(operation, uploadSource, createOp, btOp) {
    this.upload = function (requestUrl) {
        var onUploadSuccess = function (uploadOp) {
                var text = "";
                ...
                operation.uploadComplete(
                    ...
                    new BackgroundOperationWrapper(
                            new UploadOperation({
                                pendingOp: btOp,
                                started: false,
                                interface_method: "BackgroundUpload.start"
                            }),
                            btOp.guid));
            },
        ...
        tryCatchExecution(function () {
            ...
            if (btOp) {
                ...
                if (uploadSource instanceof Windows.Storage.StorageFile) {
                    asyncOp = uploader.createUpload(uploadPathUri,
                                                    uploadSource);
                    if (createOp) {
                        onCreateSuccess(asyncOp);
                        return;
                    }
                    else {
                        promise = asyncOp.startAsync().then(onUploadSuccess,
                                                            onUploadError,
                                                            onUploadProgress);
                    }
                }
...
wl_app.getCurrentBackgroundUploads = function () {
    var promise = new Promise("getCurrentBackgroundUploads", null, null);
    var ns = Windows.Networking.BackgroundTransfer;
                    ...
                    uploads.push(
                        new BackgroundOperationWrapper(
                            new UploadOperation({
                                pendingOp: response[i],
                                started: true,
                                interface_method: "BackgroundUpload.attach"
                            }),
                            response[i].guid)
                    );
                }
            }
            promise[PROMISE_EVENT_ONSUCCESS](uploads);
        },
        function (error) {
            promise[PROMISE_EVENT_ONERROR](error);
        }
...
wl_app[API_X_HTTP_LIVE_LIBRARY] = "Windows/HTML8_" + trimVersionBuildNumber("5.6.3506.0411");
